...
export const SubscriptionProvider = ({
  ...
  const stripeClient = useMemo(() => loadStripe(stripePublishableKey), [stripePublishableKey, loadStripe]);
...
export const Gate = ({ product, negate, feature, unsubscribed, children }: GateProps) => {
  ...
  if (product || feature) {
    ...
    for (let item of subscription.data) {

      if (product && item.plan.product === product.id) {

        condition = true;

      } else if (feature) {

        let productSubscribed = subscription.data.map(x => x.plan.product);
        productSubscribed = products.filter(p => productSubscribed.includes(p.product.id))

        condition = !!productSubscribed.find(q => (q.product.metadata?.features?.split(",") || []).includes(feature));
